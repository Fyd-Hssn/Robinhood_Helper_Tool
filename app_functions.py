...
def ticker_label(entry, ticker_entry):
    ...
    if len(ticker) > 4 or len(ticker) == 0:
        ...
        ticker_entry.insert(
            0,
            'You forgot to enter a Ticker Symbol! Please type it with ALL CAPS!'
        )
    ...
    if parent_name != '.!frame4':
        ...
        if not ticker:
            ...
            if strategy != 'All Strategies':
                ...
                output = main().get_options_returns_by_ticker(
                    ticker, strategy_rs)
                entry.insert(0, output)
            else:
                output = main().get_options_returns_by_ticker(ticker, strat)
...
def print_breakeven_equity(entry):
    entry.delete(0, 'end')
    if float(main().get_underlying_return()[29:]) < 0:
        output = abs(float(main().get_underlying_return()[29:])) + float(
            main().get_port_value()[22:])
        output_string = f'Your break-even equity value is ${"{:.2f}".format(output)}'
        entry.insert(0, output_string)

    elif float(main().get_underlying_return()[29:]) > 0:
        output = float(main().get_port_value()[22:]) - abs(
            float(main().get_underlying_return()[29:]))
        output_string = f'You are over your break-even! Your break-even equity value was ${"{:.2f}".format(output)}'
        entry.insert(0, output_string)


def print_combined_ticker_return(entry, ticker_entry, equity_change_result,
                                 options_return_result):
    entry.delete(0, 'end')
    ticker = ticker_entry.get()
    if not ticker:
        entry.insert(0, 'You forgot to enter a ticker symbol!')
    else:
        equity_change = float(equity_change_result.get()[32:])
        index_pointer = options_return_result.get().index('$') + 1
        options_return = float(options_return_result.get()[index_pointer:])
...
def print_ratios(menu, date, widget):
    ...
    if not watchlist or not date:
        widget.insert("1.0",
                      'You forgot to select a watchlist AND enter a date!')
